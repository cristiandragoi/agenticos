...
const client = spawn('npm.cmd', ['run', 'dev', '--', '--host', '127.0.0.1', '--port', '4180', '--strictPort'], { cwd: process.cwd(), shell: true });
...
setTimeout(() => {
  ...
  const capture = spawn('node', ['take-screenshots.js'], { cwd: process.cwd(), shell: true });
