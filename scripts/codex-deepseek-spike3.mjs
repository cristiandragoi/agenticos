// P12 spike v3 — wire_api="responses" against DeepSeek with throwaway key.
// Proves whether DeepSeek's API implements the Responses protocol Codex needs.
import { execFileSync } from 'node:child_process';
import fs from 'node:fs';
import os from 'node:os';
import path from 'node:path';

const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-spike3-'));
const workdir = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-spike3-wd-'));
