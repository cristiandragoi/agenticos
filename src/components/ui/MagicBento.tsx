...
export interface BentoCard {
  ...
  label?: string;
  color?: string; // e.g. '#120F17' or 'var(--surface-raised)'
  glowColor?: string; // Override glow color for a specific card
  onClick?: () => void;
  className?: string;
}
...
const ParticleCard = ({
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      // Let the React onClick prop fire independently — only add visual ripple
      const rect = element.getBoundingClientRect();
      const x = e.clientX - rect.left;
      const y = e.clientY - rect.top;
...
const BentoCardGrid = ({ children, gridRef }: { children: React.ReactNode; gridRef: any }) => (
...
/**
 * Subcomponent for the non-particle-card path.
 * Uses useRef + useEffect with proper cleanup to avoid accumulating
 * duplicate DOM event listeners on re-render (the bug the old ref callback had).
 */
const PlainCard: React.FC<{
  cardProps: React.HTMLAttributes<HTMLDivElement> & { style?: React.CSSProperties; className?: string };
  enableTilt: boolean;
  enableMagnetism: boolean;
  clickEffect: boolean;
...
}> = ({ cardProps, enableTilt, enableMagnetism, clickEffect, shouldDisableAnimations, currentGlowColor, children }) => {
  const cardRef = useRef<HTMLDivElement>(null);
  ...
  }, [enableTilt, enableMagnetism, clickEffect, shouldDisableAnimations, currentGlowColor]);

  return (
    <div ref={cardRef} {...cardProps}>
      {children}
    </div>
  );
};
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cards.map((card, index) => {
          ...
          const currentGlowColor = card.glowColor || glowColor;
          const cardProps = {
